...
class AppData {
  ...
  static List<Map<String, String>> sellers = [
    {"id": "77777777", "name": "Super Owner", "role": "Master Admin"},
          ...
          children: [
            ...
            Card(
              ...
              child: const Padding(
                ...
                child: Column(
                  children: [
                    Text("Live Club PK (1 vs 1)", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15)),
                    SizedBox(height: 12),
                    Row(
        ...
        child: Column(
          children: [
            Card(
              ...
              child: ListTile(
                leading: CircleAvatar(backgroundColor: Colors.pinkAccent, child: Text("👥")),
                title: Text("PB Royal Family", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                subtitle: Text("Level 8 • 154 Club Members", style: TextStyle(color: Colors.grey)),
              ),
            ),
          ],
        ),
...
class _OwnerAdminPanelScreenState extends State<OwnerAdminPanelScreen> {
  ...
  void addReseller() {
    ...
    if (id.length != 8 || name.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Enter valid 8-digit ID & Name!"), backgroundColor: Colors.red),
      );
      return;
    }

    setState(() {
      AppData.sellers.add({"id": id, "name": name, "role": "Authorized Reseller"});
    });

    newResellerIdController.clear();
    newResellerNameController.clear();

    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text("Added $name ($id) as Coin Reseller!"), backgroundColor: Colors.green),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFF0F0C20),
      appBar: AppBar(
        title: const Text("Super Owner & Reseller Panel", style: TextStyle(color: Colors.white, fontSize: 16)),
