...
class _MainNavState extends State<MainNav> {
  ...
  void _openRoom(BuildContext ctx) {
    showModalBottomSheet(context: ctx, backgroundColor: const Color(0xFF1A0D33),
      ...
      builder: (_) => Padding(padding: const EdgeInsets.all(20), child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        ListTile(leading: const Icon(Icons.mic, color: Color(0xFFFF00FF)), title: const Text("12-Seat Party Audio Room", style: TextStyle(color: Colors.white)),
          onTap: () { Navigator.pop(ctx); Navigator.push(ctx, MaterialPageRoute(builder: (_) => const RoomView())); }),
      ])));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(body: _pages[_idx],
      bottomNavigationBar: Container(height: 65, decoration: const BoxDecoration(color: Color(0xFF1A0D33), borderRadius: BorderRadius.vertical(top: Radius.circular(20))),
...
class HomeView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SafeArea(child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Expanded(child: GridView.builder(itemCount: 6,
        ...
        itemBuilder: (ctx, i) => GestureDetector(onTap: () => Navigator.push(ctx, MaterialPageRoute(builder: (_) => const RoomView())),
          child: Container(decoration: BoxDecoration(color: const Color(0xFF1A0D33), borderRadius: BorderRadius.circular(12)),
            child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              const Icon(Icons.mic, size: 40, color: Color(0xFFFF00FF)), const SizedBox(height: 8),
...
class _ProfileViewState extends State<ProfileView> {
  ...
  Future<void> _loadProfile() async {
    ...
    setState(() { globalName = prefs.getString('p_name') ?? "lij"; globalDpPath = prefs.getString('p_dp'); });
  }
  ...
  @override
  Widget build(BuildContext context) {
    const cardBg = Color(0xFF1A0D33); const pink = Color(0xFFFF00FF);
    return SafeArea(child: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(children: [
      Row(children: [
        Stack(children: [
          ...
          Positioned(bottom: 0, right: 0, child: GestureDetector(onTap: _pickDP,
            child: Container(padding: const EdgeInsets.all(4), decoration: const BoxDecoration(color: pink, shape: BoxShape.circle), child: const Icon(Icons.edit, size: 14)))),
        ]),
      ...
      Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: cardBg, borderRadius: BorderRadius.circular(14)),
...
class _RoomViewState extends State<RoomView> {
  ...
  @override
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: const Color(0xFF120826),
      appBar: AppBar(backgroundColor: Colors.transparent, title: const Text("12-Seat Party Room", style: TextStyle(fontSize: 16)),
        actions: [IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context))]),
      body: Column(children: [
        Expanded(child: Padding(padding: const EdgeInsets.all(8.0), child: GridView.builder(itemCount: 12,
          ...
          itemBuilder: (ctx, i) {
            bool occ = _seats[i] != null; String? dp = occ ? _seats[i]!['dp'] : null; bool validImg = dp != null && File(dp).existsSync();
            return GestureDetector(onTap: () => setState(() => _seats[i] = occ ? null : {'name': globalName, 'dp': globalDpPath}),
              child: Column(children: [
                Container(width: 55, height: 55, decoration: BoxDecoration(shape: BoxShape.circle,
                  color: occ ? Colors.purple : Colors.white10, border: Border.all(color: const Color(0xFFFF00FF)),
                  image: validImg ? DecorationImage(image: FileImage(File(dp)), fit: BoxFit.cover) : null),
                  child: Center(child: (!validImg) ? Text(occ ? _seats[i]!['name']!.substring(0, 1).toUpperCase() : "${i + 1}", style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold)) : null)),
                const SizedBox(height: 4), Text(occ ? _seats[i]!['name']! : "Open", style: const TextStyle(color: Colors.grey, fontSize: 10), overflow: TextOverflow.ellipsis),
              ]));
          }))),
        Container(padding: const EdgeInsets.all(10), color: const Color(0xFF1A0D33),
