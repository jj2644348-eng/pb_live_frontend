...
class _ProfileViewState extends State<ProfileView> {
  ...
  Future<void> _pickDP() async {
    ...
    if (picked != null) setState(() => _dp = File(picked.path));
  }
            ...
            Row(
              children: [
                Stack(
                  children: [
                    CircleAvatar(
                      ...
                      child: _dp == null ? const Icon(Icons.person, size: 45) : null,
                    ),
                    Positioned(
                      bottom: 0,
                      right: 0,
                ...
                const Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("lij", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                    Text("ID: 10350359", style: TextStyle(color: Colors.grey, fontSize: 12)),
                  ],
                ),
              ],
            ),
...
class _RoomViewState extends State<RoomView> {
  final List<String?> _seats = List.filled(12, null);
  @override
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          Expanded(
            child: Padding(
              ...
              child: GridView.builder(
                ...
                itemBuilder: (ctx, i) => GestureDetector(
                  onTap: () => setState(() => _seats[i] = _seats[i] == null ? "User" : null),
                  child: Column(
                    children: [
                      Container(
                        width: 55,
                        ...
                        decoration: BoxDecoration(
                          ...
                          color: _seats[i] != null ? Colors.purple : Colors.white10,
                          border: Border.all(color: const Color(0xFFFF00FF)),
                        ),
                        child: Center(child: Text(_seats[i] != null ? "Mic" : "${i + 1}", style: const TextStyle(fontSize: 12))),
                      ),
                      const SizedBox(height: 4),
                      Text(_seats[i] != null ? "Active" : "Open", style: const TextStyle(color: Colors.grey, fontSize: 10)),
                    ],
                  ),
                ),
              ),
