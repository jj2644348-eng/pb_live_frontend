...
// Central User Data Model & Database
class UserModel {
  String phone;
  String password;
  String pbId;
                  ...
                  child: Column(
                    children: [
                      CircleAvatar(
                        ...
                        child: Text(_tempAvatar, style: const TextStyle(fontSize: 40)),
                      ),
                      const SizedBox(height: 10),
                      const Text('Select Avatar / DP Icon', style: TextStyle(color: Colors.white60, fontSize: 12)),
                      const SizedBox(height: 10),
                      SizedBox(
                        height: 50,
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            return GestureDetector(
                              onTap: () => setState(() => _tempAvatar = avatars[index]),
                              child: Container(
                                margin: const EdgeInsets.symmetric(horizontal: 5),
                                padding: const EdgeInsets.all(8),
                                decoration: BoxDecoration(
                                  color: _tempAvatar == avatars[index] ? Colors.amber : const Color(0xFF221133),
...
class WalletScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Map<String, dynamic>> rechargePacks = [
      {'coins': 100, 'price': '₹90'},
      {'coins': 500, 'price': '₹450'},
      {'coins': 1200, 'price': '₹1,000'},
      {'coins': 5000, 'price': '₹4,200'},
    ];
        ...
        child: Column(
          children: [
            ...
            const Align(
              ...
              child: Text('Recharge Packs (Official Partner)', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
            ),
            const SizedBox(height: 15),
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  var pack = rechargePacks[index];
                  return Card(
                    color: const Color(0xFF221133),
                    margin: const EdgeInsets.symmetric(vertical: 8),
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                    child: ListTile(
                      leading: const Icon(Icons.toll, color: Colors.amber, size: 35),
                      title: Text('${pack['coins']} Coins', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                      subtitle: const Text('Instant Credit to ID', style: TextStyle(color: Colors.white60)),
                      trailing: ElevatedButton(
                        style: ElevatedButton.styleFrom(backgroundColor: Colors.amber),
                        onPressed: () {
                          // Show Official Recharge Number 9779353560
                          showDialog(
                            context: context,
                            builder: (context) => AlertDialog(
                              backgroundColor: const Color(0xFF221133),
                              title: const Text('Recharge Info', style: TextStyle(color: Colors.amber)),
                              content: Text(
                                'To buy ${pack['coins']} coins for ${pack['price']}, please contact official recharge agent on WhatsApp/Call:\n\n📱 9779353560',
                        ...
                        child: Text(pack['price'], style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
                      ),
                    ),
                  );
                },
...
class _ProfileTabState extends State<ProfileTab> {
  void _editProfile(BuildContext context) {
    ...
    String selectedAvatar = user.profileImage.isNotEmpty ? user.profileImage : '👤';
    final List<String> avatars = ['👤', '👦', '👧', '👨', '👩', '🦁', '👑', '💎'];
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        return GestureDetector(
                          onTap: () => setDialogState(() => selectedAvatar = avatars[index]),
                          child: Container(
                            margin: const EdgeInsets.symmetric(horizontal: 4),
                            padding: const EdgeInsets.all(6),
                            decoration: BoxDecoration(
                              color: selectedAvatar == avatars[index] ? Colors.amber : const Color(0xFF15082E),
                              borderRadius: BorderRadius.circular(8),
                            ),
                            child: Text(avatars[index], style: const TextStyle(fontSize: 20)),
                          ),
                        );
                      },
                    ),
      ...
      child: Column(
        children: [
          CircleAvatar(
            radius: 45,
            backgroundColor: Colors.amber,
            child: Text(user.profileImage.isNotEmpty ? user.profileImage : '👤', style: const TextStyle(fontSize: 45)),
          ),
          const SizedBox(height: 15),
          ...
          ElevatedButton.icon(
            ...
            label: const Text('Edit Profile & DP', style: TextStyle(color: Colors.white)),
          ),
          const SizedBox(height: 20),
          ListTile(
