...
class _RoomViewState extends State<RoomView> {
  ...
  @override Widget build(BuildContext context) => Scaffold(
    ...
    body: Column(children: [
      ...
      Expanded(child: ListView.builder(itemCount: chat.length, itemBuilder: (c, i) => Padding(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 2), child: Text(chat[i], style: const TextStyle(color: Colors.white70, fontSize: 11))))),
      Container(padding: const EdgeInsets.all(8), color: Colors.black87, child: Row(children: [
        ...
        IconButton(icon: const Icon(Icons.card_giftcard, color: Colors.purpleAccent), onPressed: () => setState(() => chat.add("🎁 Luxury Gift sent!"))),
        IconButton(icon: const Icon(Icons.send, color: Colors.pinkAccent), onPressed: () { if(ctrl.text.isNotEmpty) { setState(() => chat.add("Lovepreet: ${ctrl.text}")); ctrl.clear(); } })
      ])),
    ]),
  );
}
class WalletTab extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Scaffold(
    ...
    body: const Center(child: Text("Diamonds: 0 💎\n(Recharge via Owner Panel Only)", textAlign: TextAlign.center, style: TextStyle(color: Colors.white70, fontSize: 14))),
  );
}
