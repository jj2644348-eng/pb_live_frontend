...
// ग्लोबल फ्लोटिंग मिनी रूम स्टेट (ऐप के बाहर/다른 टैब में दिखने के लिए)
bool isFloatingRoomActive = false;
String activeRoomName = "PB Live Party Room";
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Stack(
        children: [
          _pages[_idx],
          // मिनी फ्लोटिंग रूम विजेट (जब यूजर रूम से बाहर आकर दूसरे टैब पर घूमेगा)
          if (isFloatingRoomActive && (_idx != 0))
            Positioned(
              ...
              child: GestureDetector(
                onTap: () {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const RoomView()));
                },
                child: Container(
...
class _RoomViewState extends State<RoomView> {
  ...
  void _toggleSeatLock(int index) {
    setState(() {
      if (_seats[index] == null) {
        _seats[index] = {'name': 'Locked Seat 🔒', 'isLocked': true, 'isMuted': false};
      } else if (_seats[index]!['isLocked'] == true) {
        _seats[index] = null;
      } else {
        _seats[index]!['isMuted'] = !(_seats[index]!['isMuted'] ?? false);
      }
    });
  }
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 15),
            ListTile(
              leading: Icon(isMusicPlaying ? Icons.music_note : Icons.music_off, color: Colors.pinkAccent),
              title: Text(isMusicPlaying ? "Stop Background Music" : "Play Local Room Music 🎵", style: const TextStyle(color: Colors.white)),
              onTap: () {
                setState(() => isMusicPlaying = !isMusicPlaying);
                Navigator.pop(context);
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text(isMusicPlaying ? "Music Playing in Room! 🎶" : "Music Stopped!")),
                );
              },
            ),
            ListTile(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: const Icon(Icons.add_box, color: Color(0xFFFF00FF)), // प्लस मेनू बटन
      ...
      body: Column(
        children: [
                  ...
                  return GestureDetector(
                    ...
                    child: Column(
                      children: [
                          ...
                          children: [
                            Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                color: isLocked ? Colors.red.shade900 : (occ ? Colors.purple : Colors.white10),
                        ...
                        Text(
                          isLocked ? "Locked" : (occ ? _seats[i]!['name']! : "Seat ${i + 1}"),
          ...
          // लाइव चैट / मैसेज लिस्ट व्यू (जो स्क्रीन पर कमेंट दिखाएगा)
          Expanded(
            flex: 3,
            child: Container(
          ...
          // नीचे मैसेज भेजने और गिफ्ट देने का बार
          Container(
            padding: const EdgeInsets.all(8),
            color: const Color(0xFF1A0D33),
            child: Row(
              children: [
                ...
                IconButton(
                  icon: const Icon(Icons.send, color: Color(0xFFFF00FF)),
