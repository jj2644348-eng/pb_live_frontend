    ...
    showModalBottomSheet(
      ...
      builder: (_) => Padding(
        ...
        child: Column(
          ...
          children: [
            const Text("Create Room", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white)),
    ...
    return SafeArea(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Text("PB Live Party Rooms", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
...
class MsgView extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => const SafeArea(child: Center(child: Text("Messages & Chats", style: TextStyle(fontSize: 18))));
}
...
// Global Profile Data for Permanent Sync across app
                ...
                Stack(
                  children: [
                    CircleAvatar(
                      ...
                      backgroundImage: globalDpPath != null ? FileImage(File(globalDpPath!)) : null,
                      child: globalDpPath == null ? const Icon(Icons.person, size: 45) : null,
                    ),
                    Positioned(
                      bottom: 0,
                      right: 0,
      ...
      body: Column(
        children: [
                ...
                itemBuilder: (ctx, i) {
                  ...
                  return GestureDetector(
                    onTap: () {
                      setState(() {
                        if (isOccupied) {
                          _seats[i] = null; // सीट खाली करें
                        } else {
                          // परमानेंट प्रोफाइल डेटा सीट पर बैठेगा
                          _seats[i] = {'name': globalName, 'dp': globalDpPath};
                        }
                      });
                    },
                    child: Column(
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            image: (isOccupied && _seats[i]!['dp'] != null)
                                ? DecorationImage(image: FileImage(File(_seats[i]!['dp']!)), fit: BoxFit.cover)
                                : null,
                          ),
                          child: Center(
                            child: (!isOccupied || _seats[i]!['dp'] == null)
                                ? Text(isOccupied ? _seats[i]!['name']!.substring(0, 2) : "${i + 1}", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold))
                                : null,
                          ),
                        ),
          ...
          Container(
            ...
            child: Row(
              children: [
                const Expanded(child: TextField(decoration: InputDecoration(hintText: "Say something...", border: InputBorder.none))),
                IconButton(icon: const Icon(Icons.card_giftcard, color: Color(0xFFFF00FF)), onPressed: () {}),
              ],
            ),
          ),
        ],
