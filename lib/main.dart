...
class _MainContainerState extends State<MainContainer> {
  int _currentIndex = 0;
  final List<Widget> _pages = [const HomeScreen(), const RoomsTabScreen(), const WalletTabScreen(), const ProfileTabScreen()];
...
class WalletTabScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("My Wallet & Coins 🪙", style: TextStyle(color: Colors.amber)), backgroundColor: const Color(0xFF1E193D)),
      body: Padding(
        padding: const EdgeInsets.all(16.0),
        child: Column(children: [
          Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: const Color(0xFF1E193D), borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.amber)), child: const Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text("Diamond Balance:", style: TextStyle(fontSize: 16, color: Colors.white70)),
            Text("💎 10,500", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.amber)),
          ])),
          const SizedBox(height: 20),
          const Text("Recharge Packs (Contact Owner: +91 9779353560)", style: TextStyle(color: Colors.pinkAccent, fontWeight: FontWeight.bold)),
          const SizedBox(height: 10),
          Expanded(child: ListView(children: const [
            ListTile(leading: Icon(Icons.flash_on, color: Colors.amber), title: Text("10,000 Coins Pack"), subtitle: Text("₹100 Only"), trailing: Text("Buy", style: TextStyle(color: Colors.pinkAccent))),
            ListTile(leading: Icon(Icons.flash_on, color: Colors.amber), title: Text("50,000 Coins Pack"), subtitle: Text("₹450 Only"), trailing: Text("Buy", style: TextStyle(color: Colors.pinkAccent))),
          ])),
        ]),
      ),
    );
...
class ProfileTabScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: const Text("My Profile", style: TextStyle(color: Colors.amber)), backgroundColor: const Color(0xFF1E193D)), body: const Padding(padding: EdgeInsets.all(16.0), child: Text("Lovepreet Singh | ID: 0001 | VIP 40", style: TextStyle(fontSize: 18, color: Colors.amberAccent))));
}
